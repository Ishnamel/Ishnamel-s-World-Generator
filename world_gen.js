(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    define(['world_names'], factory);
  } else if (typeof module === 'object' && module.exports) {
    module.exports = factory(require('./world_names'));
  } else {
    root.WorldGen = factory(root.WorldNames);
  }
}(typeof self !== 'undefined' ? self : this, function (WorldNames) {

  // Fallback if world_names.js was omitted
  const Lexicon = WorldNames || {
    HIERARCHY_TITLES: {
      kingdom: {
        grand: ["Kingdom of"],
        crownMed: "Royal Domain of",
        crownSmall: "Barony of",
        islandMed: "Island Duchy of",
        islandSmall: "Lordship of",
        medium: [{ type: "duchy", titles: ["Duchy of"], smallTypes: ["barony"] }]
      }
    },
    SMALL_TITLE_FORMATS: { barony: ["Barony of"] },
    THEME_POOLS: { medieval: { prefixes: ["Eld"], suffixes: ["haven"], stems: ["Eldoria"] } },
    THEME_KEYS: ["medieval"],
    HAMLET_PREFIXES: ["Little"],
    POI_DESCRIPTORS: { fort: ["Garrison"], ruins: ["Ruins"], spire: ["Spire"], mine: ["Mine"], port: ["Harbor"] },
    WATER_NAMES: { oceanPrefixes: ["Great"], oceanSuffixes: ["Ocean"], gulfTypes: ["Bay"], lakeTypes: ["Lake"], riverPatterns: ["River {name}"] }
  };
  ...
  // --- Name Generation Wrappers ---
  function generateThemedName(themeKey, rng) {
    let pool = Lexicon.THEME_POOLS[themeKey] || Lexicon.THEME_POOLS.medieval || { prefixes: ["Eld"], suffixes: ["haven"] };
  ...
  function generateName(realmTheme, rng) {
    let keys = Lexicon.THEME_KEYS || Object.keys(Lexicon.THEME_POOLS);
    let effectiveTheme = (realmTheme && rng.next() < 0.90) ? realmTheme : safeSample(keys, rng, "medieval");
  ...
  function generateTownName(tier, realmTheme, rng) {
    ...
    if (tier === "hamlet" && rng.next() < 0.65) {
      let hp = safeSample(Lexicon.HAMLET_PREFIXES, rng, "Little");
      return `${hp} ${baseName}`;
    }
    return baseName;
  }
  ...
  function generatePoiName(type, realmTheme, rng) {
    let baseName = generateName(realmTheme, rng);
    let descList = Lexicon.POI_DESCRIPTORS[type] || ["Outpost"];
    let desc = safeSample(descList, rng, "Outpost");
    return `${baseName} ${desc}`;
  }

  function generateWaterZoneName(type, nearbyCulture, rng) {
    let wNames = Lexicon.WATER_NAMES;
    let baseName = rng.next() < 0.65 ? generateThemedName(nearbyCulture, rng) : safeSample(wNames.oceanPrefixes, rng, "Azure");

    if (type === "ocean") {
      let s = safeSample(wNames.oceanSuffixes, rng, "Ocean");
      let p = safeSample(wNames.oceanPrefixes, rng, "Great");
      return rng.next() < 0.4 ? `${p} ${baseName} ${s}` : `${baseName} ${s}`;
    } else if (["gulf", "bay", "sound", "strait", "fjord", "channel"].includes(type)) {
      let t = safeSample(wNames.gulfTypes, rng, "Gulf");
      if (["Gulf", "Bay", "Strait", "Channel"].includes(t)) {
      ...
      let t = safeSample(wNames.lakeTypes, rng, "Lake");
  ...
  function generateRiverName(culture, rng) {
    ...
    let pat = safeSample(Lexicon.WATER_NAMES.riverPatterns, rng, "River {name}");
  ...
  function generateWorld(config = {}) {
    ...
    const isWilderness = config.isWilderness === true || (config.numGrandZones === 0 && config.targetPois === 0);
    const numGrandZones = isWilderness ? 0 : (config.numGrandZones !== undefined ? config.numGrandZones : 5);
    const targetRivers = config.targetRivers !== undefined ? config.targetRivers : 12;
    const targetPois = isWilderness ? 0 : (config.targetPois !== undefined ? config.targetPois : 500);
    ...
    const scale = mapType === 'archipelago' ? 0.008 : (mapType === 'giant_continent' || mapType === 'vertical_continent' ? 0.0028 : 0.0035);
    ...
    for (let y = 0; y < gh; y++) {
      ...
      for (let x = 0; x < gw; x++) {
        ...
        let dx = nx - 0.5;
        let dy = ny - 0.5;
        let centerDist = Math.sqrt(dx * dx + dy * dy) * 2;

        if (mapType === 'continents') {
          // Standard Continents
          e -= Math.pow(centerDist, 1.8) * 0.45;

        } else if (mapType === 'pangaea') {
          // Single Central Landmass
          e -= Math.pow(centerDist, 1.2) * 0.65;

        } else if (mapType === 'archipelago') {
          // Island Cluster
          e -= Math.pow(centerDist, 2.6) * 0.25;

        } else if (mapType === 'twin_continents') {
          // Twin Continents (East & West)
          let dWest = Math.sqrt(Math.pow((nx - 0.24) * 1.55, 2) + Math.pow((ny - 0.50) * 1.15, 2)) * 2.1;
          let dEast = Math.sqrt(Math.pow((nx - 0.76) * 1.55, 2) + Math.pow((ny - 0.50) * 1.15, 2)) * 2.1;
          let minDist = Math.min(dWest, dEast);

          e -= Math.pow(minDist, 1.85) * 0.48;

          let midDist = Math.abs(nx - 0.50);
          if (midDist < 0.16) {
            let trenchFactor = (1.0 - (midDist / 0.16));
            e -= trenchFactor * 0.32;
          }

        } else if (mapType === 'giant_continent') {
          // One Giant Continent (Fills landscape)
          let edgeX = Math.abs(dx) * 2.0;
          let edgeY = Math.abs(dy) * 2.0;
          let edgeDist = Math.max(edgeX, edgeY);

          if (edgeDist > 0.65) {
            let falloff = (edgeDist - 0.65) / 0.35;
            e -= Math.pow(falloff, 2.2) * 0.75;
          } else {
            e += 0.08;
          }

        } else if (mapType === 'three_continents') {
          // Three Continents (NW, NE, and South)
          let dNW = Math.sqrt(Math.pow((nx - 0.27) * 1.45, 2) + Math.pow((ny - 0.35) * 1.35, 2)) * 2.2;
          let dNE = Math.sqrt(Math.pow((nx - 0.73) * 1.45, 2) + Math.pow((ny - 0.35) * 1.35, 2)) * 2.2;
          let dS  = Math.sqrt(Math.pow((nx - 0.50) * 1.45, 2) + Math.pow((ny - 0.76) * 1.35, 2)) * 2.2;
          let minDist = Math.min(dNW, Math.min(dNE, dS));

          e -= Math.pow(minDist, 1.8) * 0.50;

          let midXDist = Math.abs(nx - 0.50);
          if (midXDist < 0.12 && ny < 0.55) {
            e -= (1.0 - (midXDist / 0.12)) * 0.28;
          }
          let midYDist = Math.abs(ny - 0.56);
          if (midYDist < 0.10) {
            e -= (1.0 - (midYDist / 0.10)) * 0.25;
          }

} else if (mapType === 'vertical_continent') {
          // --- ORGANIC VERTICAL CONTINENT WITH NORTHERN RIDGE ---
          // Domain warping: perturb coordinates to break any straight boxy lines
          let warpX = perlin.noise(nx * 3.5 + 1.2, ny * 3.5 + 4.7) * 0.18;
          let warpY = perlin.noise(nx * 3.5 + 8.1, ny * 3.5 + 2.3) * 0.18;
          let wx = nx + warpX;
          let wy = ny + warpY;

          // Smooth curved continental body (broadest in center, natural coastal bays)
          let sideDist = Math.abs(wx - 0.5) * 2.0;
          let sideFalloff = Math.pow(Math.max(0.0, (sideDist - 0.40) / 0.55), 2.2);
          e -= sideFalloff * 0.75;

          // Southern shoreline smooth taper
          if (wy > 0.72) {
            let southFalloff = Math.pow((wy - 0.72) / 0.28, 2.0);
            e -= southFalloff * 0.65;
          }

          // Northern extreme tip ocean border
          if (wy < 0.06) {
            let northEdge = Math.pow((0.06 - wy) / 0.06, 1.8);
            e -= northEdge * 0.50;
          }

          // Elevated core mainland
          e += 0.06;

          // NORTHERN MOUNTAIN WALL (Smoothly fades South with organic mountain ridges)
          if (wy < 0.36 && wy >= 0.04) {
            let northWeight = Math.cos(((wy - 0.04) / 0.32) * (Math.PI * 0.5));
            let ridgeNoise1 = perlin.fbm(x * scale * 2.5 + 500.0, y * scale * 2.5 + 200.0, 4, 0.55, 2.1);
            let ridgeNoise2 = perlin.fbm(x * scale * 5.0 + 1200.0, y * scale * 5.0 + 800.0, 3, 0.50, 2.0);
            
            let combinedRidge = (ridgeNoise1 * 0.6 + ridgeNoise2 * 0.4);
            e += (northWeight * 0.32) + (combinedRidge * northWeight * 0.26);
          }
        }
    ...
    // --- CIVILIZATION & SETTLEMENT GENERATION (SKIPPED IN WILDERNESS MODE) ---
    ...
    if (!isWilderness && numGrandZones > 0) {
      // 1. Grand Zones
   ...
   // 1. Distributed Grand Zone Placement (Spread across North, Central, and South)
      let grandRankKeys = Object.keys(Lexicon.HIERARCHY_TITLES);
      let minSpacing = (mapType === 'vertical_continent') ? 160 : 220;

      // Filter valid candidates (avoid mountains and extreme coasts)
      let candidates = landCellIndices.filter(idx => {
        let e = elevation[idx], cx = idx % gw, cy = Math.floor(idx / gw);
        let ny = cy / gh;
        // In vertical continents, keep capitals below the extreme northern mountain wall (ny >= 0.28)
        let minY = (mapType === 'vertical_continent') ? 0.28 : 0.10;
        return e >= 0.44 && e <= 0.72 && cx > gw * 0.12 && cx < gw * 0.88 && ny > minY && ny < 0.88;
      });

      if (candidates.length === 0) candidates = [...landCellIndices];

      // Score candidates by spacing, moderate flow, and regional diversity
      candidates.sort((a, b) => {
        let cyA = Math.floor(a / gw), cyB = Math.floor(b / gw);
        let scoreA = flow[a] * 0.5 + (cyA % 150) * 10;
        let scoreB = flow[b] * 0.5 + (cyB % 150) * 10;
      ...
      while (grandZones.length < numGrandZones && minSpacing >= 40) {
        for (let idx of candidates) {
          ...
          if (farEnough) {
            let gCulture = Lexicon.THEME_KEYS[(grandZones.length * 5 + Math.floor(rng.next() * 3)) % Lexicon.THEME_KEYS.length];
            let gName = generateThemedName(gCulture, rng);
            let rankType = grandRankKeys[grandZones.length % grandRankKeys.length];
            let titleList = Lexicon.HIERARCHY_TITLES[rankType].grand;
      ...
      // Organic Flood Fill (Applies micro-terrain friction so borders curve along natural features)
        ...
        for (let n of neighbors) {
          if (n.x >= 0 && n.x < gw && n.y >= 0 && n.y < gh) {
            ...
            if (!isWater[nIdx] && grandState[nIdx] === -1) {
              ...
              // Organic path cost: slope resistance + Perlin jitter breaks straight Voronoi lines
              let jitter = (perlin.noise(n.x * 0.05, n.y * 0.05) + 1.0) * 0.6;
              let moveCost = 1.0 + Math.pow(elevation[nIdx], 2.2) * 3.5 + jitter;
      ...
      for (let grand of grandZones) {
        ...
        let medDefs = Lexicon.HIERARCHY_TITLES[grand.rank].medium;
        ...
        let crownMedTitle = Lexicon.HIERARCHY_TITLES[grand.rank].crownMed || "Crownlands of";
        let primaryMed = {
          id: globalMedId,
          grandId: grand.id,
        ...
        for (let compId = 0; compId < numComponents; compId++) {
          if (compId !== grand.compId && componentSizes[compId] > 200) {
            ...
            if (compLand.length > 150) {
              ...
              let islTitle = Lexicon.HIERARCHY_TITLES[grand.rank].islandMed || "Island Province of";
      ...
      for (let compId = 0; compId < numComponents; compId++) {
        ...
        if (cCells.length >= 20 && cCells.length <= 250) {
          ...
          let sTitle = Lexicon.HIERARCHY_TITLES[pGrand.rank].islandSmall || "Lordship of";
      ...
      for (let med of medZones) {
        ...
        let sTitle0 = med.isCapitalZone
          ? (Lexicon.HIERARCHY_TITLES[parentGrand.rank].crownSmall || "Imperial Seat of")
          : safeSample(Lexicon.SMALL_TITLE_FORMATS[allowedSmallTypes[0]] || ["Barony of"], rng, "Barony of");
          ...
          while (localSmallList.length < targetSmallCount && smallDist >= 25) {
            for (let idx of mCells) {
              ...
              if (isUrbanSpaceClear(cx, cy, smallDist)) {
                ...
                let sTitleList = Lexicon.SMALL_TITLE_FORMATS[sType] || ["County of"];
      ...
      // Settlements & POIs
      pois = [...medSeats, ...smallSeats, ...placedPorts];
      ...
      const allNodesList = capitals.concat(pois);
      const componentNodesMap = new Map();

      for (let node of allNodesList) {
      ...
      componentNodesMap.forEach((compNodes) => {
        ...
        while (remainingMinors.length > 0) {
          ...
          if (rawPath && rawPath.length > 1) {
            ...
            let direct = [
              { x: m.x, y: m.y, px: m.px || (m.x * STEP + STEP / 2), py: m.py || (m.y * STEP + STEP / 2) },
              { x: bestTarget.x, y: bestTarget.y, px: bestTarget.px || (target.x * STEP + STEP / 2), py: bestTarget.py || (target.y * STEP + STEP / 2) }
            ];
            minorRoadPaths.push(direct);
      ...
      if (placedPorts.length > 0) {
        let seaEdges = [];
        for (let node of allNodesList) node.waterBodyId = getNodeWaterBody(node);
        ...
        for (let p1 of placedPorts) {
          ...
          let waterfrontSettlementsOnBody = allNodesList.filter(n => n !== p1 && n.type !== 'port' && n.waterBodyId === wb);
    ...
    // Mountain Peaks (Dense natural placement)
// Mountain Peaks (Organic spacing with local peak detection)
    let mountainPeaks = [];
    const baseStride = (mapType === 'vertical_continent') ? 34 : 26;

    for (let y = 12; y < gh - 12; y += baseStride) {
      let yOffset = y * gw;
      let ny = y / gh;
      
      // In northern mountain areas, allow slightly denser peaks but strictly spaced
      let effectiveStride = (mapType === 'vertical_continent' && ny < 0.32) ? Math.floor(baseStride * 0.85) : baseStride;

      for (let x = 12; x < gw - 12; x += effectiveStride) {
        let jx = x + Math.floor((rng.next() - 0.5) * (effectiveStride * 0.6));
        let jy = y + Math.floor((rng.next() - 0.5) * (effectiveStride * 0.6));

        if (jx >= 2 && jx < gw - 2 && jy >= 2 && jy < gh - 2) {
          let idx = jy * gw + jx;
          let e = elevation[idx];

          // Only stamp a mountain icon if it's high elevation AND a local ridge peak
          if (!isWater[idx] && e > 0.74) {
            let isLocalRidge = (
              e >= elevation[idx - 1] &&
              e >= elevation[idx + 1] &&
              e >= elevation[idx - gw] &&
  ...
  function getResolvedRealmString(worldData, target) {
    if (!worldData || !target || worldData.isWilderness) return "Wilderness";
    ...
    let gRealm = worldData.grandZones ? worldData.grandZones[worldData.grandState[cellIdx]] : null;
    let mRealm = worldData.medZones ? worldData.medZones[worldData.medState[cellIdx]] : null;
    let sRealm = worldData.smallZones ? worldData.smallZones[worldData.smallState[cellIdx]] : null;
  ...
  function drawSmoothVectorBorders(ctx, worldData, opts = {}) {
    if (worldData.isWilderness) return;
    ...
    (grandZones || []).forEach(z => grandMap.set(z.id, z));
    const medMap = new Map();
    (medZones || []).forEach(z => medMap.set(z.id, z));
    const smallMap = new Map();
    (smallZones || []).forEach(z => smallMap.set(z.id, z));
      ...
      for (let y = 0; y < gh - 1; y++) {
        ...
        for (let x = 0; x < gw - 1; x++) {
          ...
          if (isWater[idx] !== isWater[right] || (!isWater[idx] && smallState[idx] !== smallState[right])) {
            ...
            const sz = smallMap.get(smallState[sIdx]);
            if (sz && sz.color) {
              ctx.strokeStyle = `rgb(${sz.color.borderR}, ${sz.color.borderG}, ${sz.color.borderB})`;
              ctx.beginPath();
      ...
      for (let y = 0; y < gh - 1; y++) {
        ...
        for (let x = 0; x < gw - 1; x++) {
          ...
          if (isWater[idx] !== isWater[right] || (!isWater[idx] && medState[idx] !== medState[right])) {
            ...
            const mz = medMap.get(medState[mIdx]);
            if (mz && mz.color) {
              ctx.strokeStyle = `rgb(${mz.color.borderR}, ${mz.color.borderG}, ${mz.color.borderB})`;
              ctx.beginPath();
              ctx.moveTo((x + 1) * STEP, y * STEP);
              ctx.lineTo((x + 1) * STEP, (y + 1) * STEP);
              ctx.stroke();
      ...
      for (let y = 0; y < gh - 1; y++) {
        ...
        for (let x = 0; x < gw - 1; x++) {
          ...
          if (isWater[idx] !== isWater[right] || (!isWater[idx] && grandState[idx] !== grandState[right])) {
            ...
            const gz = grandMap.get(grandState[gIdx]);
            if (gz && gz.color) {
              ctx.strokeStyle = `rgb(${gz.color.borderR}, ${gz.color.borderG}, ${gz.color.borderB})`;
              ctx.beginPath();
              ctx.moveTo((x + 1) * STEP, y * STEP);
              ctx.lineTo((x + 1) * STEP, (y + 1) * STEP);
              ctx.stroke();
  ...
  function renderTerrainToCanvas(canvas, worldData, opts = {}) {
    ...
    for (let y = 0; y < height; y++) {
      ...
      for (let x = 0; x < width; x++) {
        ...
        if (!isWater[cellIdx] && !worldData.isWilderness) {
          let zColor = null;
    ...
    // Roads (Civilization only)
    if (showRoads && !worldData.isWilderness) {
      ctx.save();
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
  ...
  function renderFeaturesToCanvas(canvas, worldData, opts = {}) {
    if (!canvas || !worldData) return;
    const ctx = canvas.getContext('2d');
    const {
      gw, gh, STEP, width, height,
    ...
    // 1. Vector Borders (Civilization only)
    if (!worldData.isWilderness) {
      drawSmoothVectorBorders(ctx, worldData, { showGrand, showMed, showSmall });
    }
    ...
    // 6. Settlements (Civilization only)
    if (!worldData.isWilderness) {
      for (let cap of (capitals || [])) {
        let px = cap.px || (cap.x * STEP + STEP / 2);
        let py = cap.py || (cap.y * STEP + STEP / 2);
  ...
  function buildSvgOverlay(worldData, opts = {}) {
    ...
    if (!worldData.isWilderness) {
      // Grand Zone Titles
      if (showGrandLbl && grandZones) {
        out += `<g id="svg-grand-titles">`;
        for (let grand of grandZones) {
          if (!grand.capital || grand.cellCount === 0) continue;
      ...
      // Medium Zone Titles
      if (showMedLbl && medZones) {
        out += `<g id="svg-med-titles">`;
        for (let med of medZones) {
          if (!med.capital || med.cellCount === 0) continue;
      ...
      // Small Zone Titles
      if (showSmallLbl && smallZones) {
        out += `<g id="svg-small-titles">`;
        for (let small of smallZones) {
          if (!small.capital || small.cellCount === 0) continue;
    ...
    // Settlement Labels (Civilization only)
    if (!worldData.isWilderness) {
      out += `<g id="svg-poi-labels">`;
      if (showCapitalLbl && capitals) {
        for (let cap of capitals) {
          let px = cap.px || (cap.x * STEP + STEP / 2);
          let py = cap.py || (cap.y * STEP + STEP / 2);
